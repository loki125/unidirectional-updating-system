                                                   ...
                                                   const std::string& architecture) 
{
    ...
    if (!outfile.is_open()) {
        throw std::runtime_error("Failed to open local file for writing: " + file_name);
    }

    auto res = client->Get(endpoint.c_str(), [&](const char *data, size_t data_length) {
    ...
    if (!res || res->status != 200) {
        std::remove(file_name.c_str()); // Clean up corrupted download
        std::string err_msg = "Failed to download deb: ";

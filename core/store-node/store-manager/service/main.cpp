...
void initialize_environment() {
    try {
        fs::path output_path = fs::path(get_env_var("OUTPUT_PATH")) / READY_PATH;
        fs::path store_path  = fs::path(get_env_var("STORE_PATH")) / PROCESSING_DIR;
        ...
        std::string router_ip = get_env_var("ROUTER_MCAST_IP");
        std::string view_ip = get_env_var("VIEW_IP");
        exec_command("ip route add " + view_ip +" via " + router_ip);

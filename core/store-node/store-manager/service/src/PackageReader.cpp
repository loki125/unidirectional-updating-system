...
void DebReader::bundle_system_package(fs::path store_vol, const std::string& sys_pkg_deb_path, const std::vector<json>& subgraph) {
    ...
    spdlog::debug("Bundling dependencies into system package hash path at: {}", sys_store_path.string());
    ...
    for (const auto& dep : subgraph) {
        ...
        spdlog::debug("Hard-linking dependency {} into bundle", dep[pkg::NAME].get<std::string>());
        ...
        try {
            ...
            if (fs::is_symlink(dep_deb_path)) {
                ...
                spdlog::debug("Copied symlink {} to {}", dep_deb_path.filename().string(), destination.string());
            } else {
                // Create a Hard Link: (Existing_File, New_Link)
                fs::create_hard_link(dep_deb_path, destination);
                spdlog::debug("Hard-linked {} into bundle", dep_deb_path.filename().string());
            }
        } catch (const fs::filesystem_error& e) {
            spdlog::error("Failed to bundle dependency {}: {}", dep_deb_path.string(), e.what());
        }
    }
    spdlog::debug("Successfully bundled all recursive dependencies into {}", sys_store_path.filename().string());
}
